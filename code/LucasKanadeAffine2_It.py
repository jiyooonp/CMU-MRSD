...
def LucasKanadeAffine(It, It1, threshold, num_iters):
    ...
    while True:
        ...
        It_w = affine_transform(It, M)
        # print("It1_w.shape", It1_w.shape)
        # plt.imshow(It1_w_interp_patch, cmap='gray')
        # plt.show()

        It_w_interp = RectBivariateSpline(np.arange(It1.shape[0]), np.arange(It1.shape[1]), It_w)

        it_w_x = np.linspace(0, It.shape[0], It.shape[0]) # 480
        it_w_y = np.linspace(0, It.shape[1], It.shape[1]) # 640

        # print(it1_w_x.shape)
        # print(it1_w_y.shape)

        x1_stack = np.dstack([it_w_x] * it_w_y.shape[0])
        y1_stack = np.dstack([it_w_y] * it_w_x.shape[0]).T
        It_w_interp_patch = It_w_interp.ev(y1_stack[:, :, 0], x1_stack[0]) # 480 x 640
        # print(f"It1_w_interp_patch.shape: {It1_w_interp_patch.shape}")

        d_It1_w_x = It_w_interp.ev(y1_stack[:, :, 0], x1_stack[0], dy=1).flatten() #307200
        d_It1_w_y = It_w_interp.ev(y1_stack[:, :, 0], x1_stack[0], dx=1).flatten() #307200

        It1_w_x = It_w_interp.ev(y1_stack[:, :, 0], x1_stack[0]).flatten()  # 307200
        It1_w_y = It_w_interp.ev(y1_stack[:, :, 0], x1_stack[0]).flatten()  # 307200
        ...
        D = It1.flatten() - It_w.flatten() # wh * 1
