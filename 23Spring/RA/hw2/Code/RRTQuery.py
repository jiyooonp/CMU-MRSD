...
envpoints, envaxes = rt.BlockDesc2Points(rt.rpyxyz2H([0,0.,0.],[0.1,0,1.0]),[1.3,1.4,0.1]) # top
pointsObs.append(envpoints), axesObs.append(envaxes)

envpoints, envaxes = rt.BlockDesc2Points(rt.rpyxyz2H([0,0.,0.],[0.1,-0.65,0.475]),[1.3,0.1,0.95]) # left, side
pointsObs.append(envpoints), axesObs.append(envaxes)

envpoints, envaxes = rt.BlockDesc2Points(rt.rpyxyz2H([0,0.,0.],[0.1, 0.65,0.475]),[1.3,0.1,0.95]) # right, side
pointsObs.append(envpoints), axesObs.append(envaxes)

envpoints, envaxes = rt.BlockDesc2Points(rt.rpyxyz2H([0,0.,0.],[-0.5, 0, 0.475]),[0.1,1.2,0.95]) # back
pointsObs.append(envpoints), axesObs.append(envaxes)
	...
	while len(rrtVertices)<3000 and not FoundSolution:
		...
		while np.linalg.norm(qRand - qNear) > thresh:
			...
			if not mybot.DetectCollisionEdge(qConnect, qNear, pointsObs, axesObs):
				...
				break
		# 3. Extend the tree towards the random configuration
		# 4. Check for collision
		# 5. If no collision, add the new configuration to the tree
		qConnect = np.array(qRand)
		if not mybot.DetectCollisionEdge(qConnect, qNear, pointsObs, axesObs):
			rrtVertices.append(qConnect)
			rrtEdges.append(idNear)

		# 7. If close to goal, set FoundSolution to True
