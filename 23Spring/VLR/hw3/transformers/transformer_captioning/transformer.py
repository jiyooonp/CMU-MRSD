...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.1):
        ...
        # TODO: Initialize the following layers and parameters to perform attention
        self.head_proj = nn.Linear(embed_dim, embed_dim * num_heads, bias=False)
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = query.view(N, S, H, D // H)
        key = key.view(N, T, H, D // H)
        value = value.view(N, T, H, D // H)

        # transpose to get (N, H, S, D/H)
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        # pe = torch.zeros_like(torch.Tensor(x.shape))
        # x = x.to(torch.long).cuda()
        # for i in range(N):
        #     pe[i, :, :] = self.encoding(x[i, :, :])
        # output = x + self.encoding(x)
        output = x + pe #torch.autograd.Variable(pe[:, :S, :], requires_grad=False)
...
class TransformerDecoder(nn.Module):
    ...
    def get_data_embeddings(self, features, captions):
        ...
        # caption_embedding += self.positional_encoding(captions)
