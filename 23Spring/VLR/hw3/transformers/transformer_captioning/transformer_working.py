...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        if attn_mask is not None:
            ...
            # additive_mask = attn_mask - 1e20
            # additive_mask[additive_mask != - 1e20] = 0
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # for i in range(H):
        #     if i == 0:
        #         y = super(MultiHeadAttentionLayer, self).forward(query[:, :, 0, :], key[:, :, 0, :], value[:, :, 0, :])
        #     else:
        #         y = torch.cat((y, super(MultiHeadAttentionLayer, self).forward(query[:, :, i, :], key[:, :, i, :], value[:, :, i, :])), dim=2)
