...
def fcos_get_deltas_from_locations(
    ...
    #     else:
    #         deltas[i, :] = torch.tensor([xc - x1, yc - y1, xc - x2, yc - y2])/stride
    for i in range(N):
        x, y = locations[i, :]
        x0, y0, x1, y1 = gt_boxes[i, :4]
        if torch.equal(gt_boxes[i, :4], torch.tensor([-1, -1, -1, -1])):
...
def fcos_apply_deltas_to_locations(
    ...
    for i in range(N):
        xc, yc = locations[i, :]
        x0, y0, x1, y1 = deltas[i, :4] * stride

        if torch.equal(deltas[i, :4],torch.tensor([-1, -1, -1, -1])):
            output_boxes[i, :] = torch.tensor([[xc, yc, xc, yc]]) # had to debug & print to figure this out
        else:
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    for i in range(N):
        ...
        if [l, t, r, b]==[-1, -1, -1, -1]:
            ...
            center = torch.sqrt((min(l, r)* min(t, b))/ (max(l, r) * max(t, b)))
        centerness[i] = center
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def get_fpn_location_coords(
    ...
    location_coords = {
        level_name: torch.zeros((N, 2)) for level_name, _ in shape_per_fpn_level.items()
    }
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        for x in range(W):
            for y in range(H):
                coords[i, :] = torch.tensor([x*level_stride + math.floor(level_stride/2), y*level_stride + math.floor(level_stride/2)])
                i +=1
