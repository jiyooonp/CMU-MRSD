...
class TestDetectorBackboneWithFPN(unittest.TestCase):
    ...
    def test_prediction_network(self):
        ...
        # print("iniput shape: ",self.dummy_fpn_feats["p3"].shape)
    ...
    def test_gt_targets_for_box_regression(self):
        ...
        # print(f"input boxes: {input_boxes[:, :4]} \noutput boxes: {output_boxes}")
    ...
    def test_centerness_and_regression_loss(self):
        ...
        # print(f"dummy: {dummy_gt_deltas} \n real: {torch.tensor([[ 0.9688,  0.9375,  0.2500,  0.5625],[-1.0000, -1.0000, -1.0000, -1.0000]])}")
        if dummy_gt_deltas is None:
            self.skipTest("fcos_get_deltas_from_test is not implemented.")
        else:
            print("dummy_gt_deltas.device", dummy_gt_deltas.device)
