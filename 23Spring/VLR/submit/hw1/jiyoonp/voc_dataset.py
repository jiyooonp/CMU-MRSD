    ...
    def preload_anno(self):
        ...
        for index in self.index_list:
            ...
            for object in root.findall('object'):
                ...
                cls_diff = int(object.find('difficult').text)
                cls_index = self.get_class_index(cls_name)
                if cls_diff == 1:
                    difficulty[cls_index] = list(map(add, difficulty.get(cls_index, [0, 0]), [0, 1]))
                else:
                    difficulty[cls_index] = list(map(add, difficulty.get(cls_index, [0, 0]), [1, 0]))
            # print(difficulty)
            for i in range(len(self.CLASS_NAMES)):
                if difficulty.get(i, [-1, -1])[0] >= difficulty.get(i, [-1, -1])[1]:
                    weight_vec[i] = 1
                else:
                    weight_vec[i] = 0
                # but then what happens if there are two of the same object that has one difficult and one not difficult?
            # print(weight_vec)
