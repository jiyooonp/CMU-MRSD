...
def fcos_match_locations_to_gt(
    ...
    for level_name, centers in locations_per_fpn_level.items():
        ...
        matched_gt_boxes[level_name] = matched_boxes_this_level.to(device="cuda:0")
...
def fcos_get_deltas_from_locations(
    ...
    for i in range(N):
        x, y = locations[i, :]
        if gt_boxes[i, 0] == -1:
            pairwise_dist = torch.tensor([-1, -1, -1, -1])
        else:
            pairwise_dist = torch.tensor([x - gt_boxes[i,0], y - gt_boxes[i,1], gt_boxes[i,2] - x, gt_boxes[i,3] - y])/stride
        deltas[i, :] = pairwise_dist
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    for i in range(N):
        x, y = locations[i, :]
        deltas *= stride
        x0, y0, x1, y1 = deltas[i][0], deltas[i][1], deltas[i][2], deltas[i][3]

        if x0 == -1*stride:
            output_boxes[i, :] = torch.tensor([-1, -1, -1, -1])
        else:
            output_boxes[i, :] = torch.clamp(torch.tensor([[x-x0, y-y0, x+x1, y+y1]]), min=0) # had to debug & print to figure this out
        # else:
        #     output_boxes[i, :] = torch.tensor([xc-x0, yc-y0, xc+x1, yc+y1])
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################

    return output_boxes.to(device=0)
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    for i in range(N):
        if deltas[i][0]==-1:
            ...
            center = torch.sqrt((torch.min(deltas[i][0], deltas[i][2])* torch.min(deltas[i][1], deltas[i][3]))
                                / (torch.max(deltas[i][0], deltas[i][2]) * torch.max(deltas[i][1], deltas[i][3])))
        centerness[i] = center
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################

    return centerness.to(device=0)
...
def get_fpn_location_coords(
    ...
    # print("$$$", shape_per_fpn_level["p3"], len(shape_per_fpn_level))
    B, C, H, W = shape_per_fpn_level["p3"]
    N = H*W

    location_coords = {
        level_name: torch.zeros((N, 2)).to(device) for level_name, _ in shape_per_fpn_level.items()
    }
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        ######################################################################
        coords = torch.zeros((N, 2), device=0)
        i = 0
        for x in range(W):
            for y in range(H):
                coords[i, :] = torch.tensor([
                        x*level_stride + math.floor(level_stride/2),
                        y*level_stride + math.floor(level_stride/2)
                    ],device=0)
                i +=1
        location_coords[level_name] = coords
        # print("location_coords[level_name]", location_coords[level_name])
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
