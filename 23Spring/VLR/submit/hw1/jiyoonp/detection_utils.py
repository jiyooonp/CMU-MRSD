...
def fcos_match_locations_to_gt(
    ...
    for level_name, centers in locations_per_fpn_level.items():
        ...
        x, y = centers.to(device=0).unsqueeze(dim=2).unbind(dim=1)
...
def fcos_get_deltas_from_locations(
    ...
    for i in range(N):
        x, y = locations[i, :]
        x0, y0, x1, y1 = gt_boxes[i, :4]
        if x0 == -1:
            pairwise_dist = torch.tensor([-1, -1, -1, -1])
        else:
            pairwise_dist = torch.tensor([x - x0, y - y0, x1 - x, y1 - y])/stride
...
def fcos_apply_deltas_to_locations(
    ...
    for i in range(N):
        xc, yc = locations[i, :]
        x0, y0, x1, y1 = deltas[i][0] * stride, deltas[i][1] * stride, deltas[i][2] * stride, deltas[i][3] * stride
        if x1 == -1*stride:
            output_boxes[i, :] = torch.tensor([xc, yc, xc, yc])
        else:
            output_boxes[i, :] = torch.clamp(torch.tensor([[xc-x0, yc-y0, xc+x1, yc+y1]]), min=0) # had to debug & print to figure this out
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        for x in range(W):
            for y in range(H):
                coords[i, :] = torch.tensor([x*level_stride + math.floor(level_stride/2), y*level_stride + math.floor(level_stride/2)]).to(device)
                i +=1
