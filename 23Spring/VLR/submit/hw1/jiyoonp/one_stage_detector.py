...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        # print("dummy_out_shapes", dummy_out_shapes)
        ## dummy_out_shapes [('c3', torch.Size([2, 64, 28, 28])), ('c4', torch.Size([2, 160, 14, 14])), ('c5', torch.Size([2, 400, 7, 7]))]
        ...
        self.lateral1 = nn.Conv2d(dummy_out_shapes[0][1][1], self.out_channels , 1, stride=1, padding ='same')
        self.lateral2 = nn.Conv2d(dummy_out_shapes[1][1][1], self.out_channels , 1, stride=1, padding ='same')
        self.lateral3 = nn.Conv2d(dummy_out_shapes[2][1][1], self.out_channels , 1, stride=1, padding ='same')

        self.output1 = nn.Conv2d(self.out_channels , self.out_channels , 3, stride=1, padding='same')
        self.output2 = nn.Conv2d(self.out_channels , self.out_channels , 3, stride=1, padding='same')
        self.output3 = nn.Conv2d(self.out_channels , self.out_channels , 3, stride=1, padding='same')

        self.fpn_params = nn.ModuleDict({
            'lateral1':self.lateral1,
            'lateral2':self.lateral2,
            'lateral3':self.lateral3,
    ...
    def forward(self, images: torch.Tensor):
        ...
        # fpn_feats["p3"] = self.lateral1(backbone_feats["c3"])
        # fpn_feats["p3"] = self.output1(fpn_feats["p3"])
        # fpn_feats["p4"] = self.lateral2(backbone_feats["c4"])
        # fpn_feats["p4"] = self.output2(fpn_feats["p4"])
        # fpn_feats["p5"] = self.lateral3(backbone_feats["c5"])
        # fpn_feats["p5"] = self.output3(fpn_feats["p5"])
        #
        # fpn_feats["p5"] = F.interpolate(fpn_feats["p5"], scale_factor=4, mode="nearest")
        # fpn_feats["p4"] = F.interpolate(fpn_feats["p4"], scale_factor=2, mode="nearest")

        fpn_feats["p5"] = self.lateral3(backbone_feats["c5"])
        fpn_feats["p5"] = self.output1(fpn_feats["p5"])

        fpn_feats["p4"] = self.lateral2(backbone_feats["c4"]) + F.interpolate(fpn_feats["p5"], scale_factor=2, mode='nearest')
        fpn_feats["p4"] = self.output2(fpn_feats["p4"])

        fpn_feats["p3"] = self.lateral1(backbone_feats["c3"]) + F.interpolate(fpn_feats["p4"], scale_factor=2, mode="nearest")
        fpn_feats["p3"] = self.output3(fpn_feats["p3"])
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        stem_box = [
            nn.Conv2d(in_channels, stem_channels[1], kernel_size=3, stride = 1, padding=1),
            nn.ReLU(),
            nn.Conv2d(in_channels, stem_channels[1], kernel_size=3, stride = 1, padding=1),
            nn.ReLU(),
            nn.Conv2d(in_channels, stem_channels[1], kernel_size=3, stride = 1, padding=1),
        ...
        self.pred_cls = nn.Conv2d(stem_channels[-1], num_classes, kernel_size=3, stride=1,padding='same')  # Class prediction conv
        self.pred_box = nn.Conv2d(stem_channels[-1], 4, kernel_size=3, stride=1,padding='same')  # Box regression conv
        self.pred_ctr = nn.Conv2d(stem_channels[-1], 1, kernel_size=3, stride=1,padding='same')  # Centerness conv
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key, feature in feats_per_fpn_level.items():
            class_logits[key] = self.stem_cls(feature)
            class_logits[key] = self.pred_cls(class_logits[key])
            class_logits[key] = class_logits[key].contiguous().view(class_logits[key].size(0), class_logits[key].size(1),-1).permute(0, 2, 1)

            boxreg_deltas[key] = self.stem_box(feature)
            boxreg_deltas[key] = self.pred_box(boxreg_deltas[key])
            boxreg_deltas[key] = boxreg_deltas[key].contiguous().view(boxreg_deltas[key].size(0), boxreg_deltas[key].size(1),-1).permute(0, 2, 1)

            centerness_logits[key] = self.stem_box(feature)
            centerness_logits[key] = self.pred_ctr(feature)
            centerness_logits[key] = centerness_logits[key].contiguous().view(centerness_logits[key].size(0), centerness_logits[key].size(1),-1).permute(0, 2, 1)
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        # print("pred_cls_logits", pred_cls_logits["p3"].shape, pred_cls_logits["p3"])
        ...
        matched_gt_boxes = [] # B x N x 4
     
        for i in range(gt_boxes.size(0)):
            fcos_match = fcos_match_locations_to_gt(
                locations_per_fpn_level,
                self.backbone.fpn_strides,
                gt_boxes[i])
            matched_gt_boxes.append(fcos_match)
        ...
        for i in range(len(matched_gt_boxes)):
            deltas_temp = {}
            for key, val in self.backbone.fpn_strides.items():
                deltas_temp[key] = fcos_get_deltas_from_locations(
                        locations_per_fpn_level[key], 
                        matched_gt_boxes[i][key],
                        self.backbone.fpn_strides[key]
                    )
            matched_gt_deltas.append(deltas_temp)
        ...
        ######################################################################
        # Feel free to delete this line: (but keep variable names same)
        gt_classes = matched_gt_boxes[:,:,-1]                # N, 1

        gt_class_mask = torch.where(gt_classes>0)               # gets index where it has classes
        gt_one_hot = torch.zeros(matched_gt_boxes.size(0), matched_gt_boxes.size(1), self.num_classes, device =0) # N, num_classes

        for i in range(gt_class_mask[0].size(0)):
            b, index = gt_class_mask[0][i], gt_class_mask[1][i]
            need = int(gt_classes[b,index].item())
            need = torch.tensor([need], device=0)
            gt_one_hot[b][index] = torch.nn.functional.one_hot(need, num_classes=self.num_classes)
        # gt_one_hot[gt_class_mask, :] = torch.nn.functional.one_hot(gt_class_mask, num_classes=self.num_classes)


        # This loss expects logits, not probabilities (DO NOT apply sigmoid!)  sigmoid_focal_loss
        ...
        # gt_logits = torch.zeros_like(pred_cls_logits)
        # gt_logits[torch.arange(gt_logits.shape[0]), gt_classes] = 1

        loss_cls = sigmoid_focal_loss(inputs=pred_cls_logits, targets=gt_one_hot.view(-1, 20))

        # Multiply with 0.25 to average across four LTRB components.
        loss_box = 0.25 * F.l1_loss(
            pred_boxreg_deltas.to(device=matched_gt_deltas.device), matched_gt_deltas, reduction="none"
        )
        ...
        # calculate gt_ctr
        gt_ctr = fcos_make_centerness_targets(matched_gt_deltas.view(-1, 4)) # (B, C, 5)

        pred_ctr_logits = pred_ctr_logits.view(-1)
        loss_ctr = F.binary_cross_entropy_with_logits(
            pred_ctr_logits.to(device=gt_ctr.device), gt_ctr, reduction="none"
        )
        # No loss for background:
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            level_pred_scores = level_pred_scores[:, torch.argmax(level_pred_scores, dim=1)][:, 0]

            # Step 2:
            # Replace "pass" statement with your code
            higher = level_pred_scores > test_score_thresh

            # Step 3:
            # Replace "pass" statement with your code
            level_pred_boxes = fcos_apply_deltas_to_locations(level_deltas, level_locations, self.backbone.fpn_strides[level_name])
            level_pred_boxes = level_pred_boxes[higher]


            # Step 4: Use `images` to get (height, width) for clipping.
            # Replace "pass" statement with your code
            b, c, h_limit, w_limit = images.size()
            for i in range(level_pred_boxes.size(0)):
                x1, y1, x2, y2 = level_pred_boxes[i]
                x1, x2 = self.clipped(x1, h_limit), self.clipped(x2, h_limit)
                y1, y2  = self.clipped(y1, w_limit), self.clipped(y2, w_limit)
                level_pred_boxes[i] = torch.tensor([x1, y1, x2, y2])
